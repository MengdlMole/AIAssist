...
- 快速档优先在对话中输出，通常不生成覆盖统计或完整模板；保留主流程、关键节点、关键日志、配置项汇总和限制即可。
- 标准档强制保留范围与结论、业务时序图、调用树、关键节点、关键日志、配置项汇总、分支/边界/未知；配置项为空也保留一句明确结论。完整 reference 只在节点、日志和配置表详细出现。
- 简单同步链的标准档通常可控制在约 80–150 行；这只是去重信号而非硬限制。明显超出时先合并重复的范围、配置、边界和证据索引，不得删减必要证据或硬截断。
- 简单流程可以合并“数据流”和“边界”小节，但不得删除关键日志、配置项汇总和未知项结论。
- 复杂流程优先拆为同步主时序与异步/异常子图，不要在一个 Mermaid 图中塞入全部参与者和分支。
- 用户只需快速走读时，也保留精简业务时序图和精简代码调用树；摘要、关键节点、关键日志、配置项汇总和未知项不可省略，其余表格按需压缩。
