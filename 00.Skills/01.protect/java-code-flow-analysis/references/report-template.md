...
- 数据库标记只放在直接执行操作的节点，格式固定为 `[DB <OP> <table>]`；时序图、调用树和表操作汇总必须一致，详细 SQL 不塞入图中。
...
sequenceDiagram
    ...
    alt <失败条件>
        ...
        S->>DB: N1.1.2 <关键读写> [DB SELECT <table>] · <Repository#35;method>
        DB-->>S: 返回 N1.1.2
...
└── N1.1 [标签] <Class#method> — <职责> {代码已证实}
    ...
    └── [否则]
        ├── N1.1.2 [DB] <Class#method> — <职责> [DB SELECT <table>] {代码已证实}
...
| 节点 | 表 | 操作 | 业务目的 | 条件/关键字段 | 事务/结果影响 | SQL/映射证据 | 核心性/理由 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| N1.1.2 | `<schema.table>` | `SELECT` | `<读取业务对象>` | `<WHERE 条件；主键/业务键>` | `<所在事务；结果如何影响后续>` | `<SQL/XML/annotation reference；Mapper callsite>` | `<核心/辅助；理由>` |
...
- [ ] 分支、异步、事务、外部和异常边界已标注。
- [ ] 所有可达数据库操作均解析到实际表或显式 unresolved；时序图和调用树在直接操作节点附有一致的 `[DB <OP> <table>]`。
