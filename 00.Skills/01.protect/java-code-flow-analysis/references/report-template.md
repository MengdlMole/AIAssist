...
└── N1.1 [标签] <Class#method> — <职责> {代码已证实}
    ...
    └── [否则]
        ├── N1.1.2 [DB] <Class#method> — <职责> {代码已证实}
        └── N1.1.3 [消息] <Class#method> — <职责> {代码已证实}
```

## 6. 关键代码节点

节点表是编号身份的唯一详细定义。短符号必须与调用树、时序图首次调用消息一致；父节点后的 reference 是实际 callsite。同一短符号和 callsite 因分支、事务或线程上下文不同而分别编号时，在该单元格追加 `；context=<简短上下文>`。
...
| N1.1.2 | `<Repository#method>` — `<关键读写>` | `N1.1；<path:callsite-line>` | `<path:line · fqcn#method(types)>` | `<成功条件；类型 -> 类型>` | `<DB 边界>` | `<代码已证实；配置绑定>` |
...
| --- | --- | --- | --- | --- | --- | --- |
| L1 / N1.1 | `<path:line · symbol>`；`<code/aspect/...>`；`<INFO/...>` | `<arrival/.../failure>`；`<before/.../on-exception + 相对对象；异常机制按需>` | `<源码稳定模板>`；`<traceId/orderId/...>` | `<代码分支 + 日志级别/pointcut；配置未知则说明>` | `<实际证明>` / `<事务提交、消费完成等不可推出事实>` | `<代码已证实；项目源码>`；`<无/敏感风险>` |
...
| --- | --- | --- | --- |
| `<profile/qualifier/feature flag/topic/...>` | `<N1.1.2>` | `<选择实现/改变分支>` | `<配置位置或未知>` |
...
- 标准档只强制保留范围与结论、业务时序图、调用树、关键节点、关键日志、分支/边界/未知六类内容；空章节删除，数据流、配置和证据索引能并入相邻表时不独立成节。完整 reference 只在节点/日志表详细出现。
